...
空のno-op Senseを学習用サンプルとして使わないでください。実用的なSenseは`ICaptiveDetector`、`IItemDetector`、`IChainpointDetector`、`ISuspicionCheckpointDetector`、`IPatrollerDetector`などのDetector Interfaceを実装します。完全に動く例は`44_SenseコードMod.md`を参照してください。
