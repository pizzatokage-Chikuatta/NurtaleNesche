...
Orc のコントローラーや Nesche のアニメーションセットなど、特定のアニメーションセットを対象にする時に使います。
...
2. "target" はアニメーションターゲットIDです。patrollerデータIDではありません。例: "Orc_AC Edit"、"Nesche"。
3. "variant" は基本 "" です。Nescheの拘束バリアントでは "armbinder"、"ccc"、"yoke_bondage" などを正確に指定します。
...
7. アニメーションModでは "patroller.goblin.stage_06" のようなIDをtargetに使ってはいけません。patroller IDはゲームプレイデータIDであり、アニメーションターゲットではありません。
