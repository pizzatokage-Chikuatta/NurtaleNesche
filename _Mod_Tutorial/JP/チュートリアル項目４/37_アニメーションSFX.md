...
`animationClipId`は`_animations.json`内のclip idです。`target`でも`variant`でもcontrollerの`stateId`でもありません。Orc override用の自作clipにSFXを付ける場合は自作clipIdを使います。Nesche patch対象にSFXを付ける場合は、patchしている内蔵Nesche clipIdを使います。
