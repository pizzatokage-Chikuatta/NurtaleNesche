...
Hシーンを再生できるPatrollerには、もう一つ重要な設定が必要です。

そのPatrollerがゲーム内Hシーンを再生する場合、Prefab側に有効なHシーン用State経路が必要です。通常のBaseController系の敵では、基本的に以下が必要です。

1. Unity Inspectorで"BaseController.baseInGameHState"が設定されている。
2. 設定された"InGameHState"の"hsceneControls"リストが埋まっている。
3. そのPatrollerのTask Optionが使う"hIndex"ごとに、対応するHSceneControlがリスト内に存在する。
...
Slime、Golem、Tentacleのように専用Controller/State構造を持つPrefabを元にする場合は、空のStateへ置き換えず、そのPrefab固有のHSceneControl設定を維持してください。

この設定が抜けていると、PatrollerがPlayerを捕まえるところまでは成功しても、用意したHアニメーションが開始しないことがあります。
...
8. Playerを捕まえるがHシーンが始まらない: "BaseController.baseInGameHState"、その"hsceneControls"リスト、Task Option側の"hIndex"を確認する。"[InGameHState] Missing HSceneControl index ..."のようなConsole警告が出る場合、リストが空か、指定Indexが範囲外です。
