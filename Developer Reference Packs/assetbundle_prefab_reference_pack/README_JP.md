...
ゲーム内Hシーンを再生できるPatrollerサンプルPrefabでは、Hシーン設定を維持してください。特にBaseControllerの"baseInGameHState"と、関連するHSceneControlリスト、またはController固有の代替設定を確認します。このInspector設定が欠けると、Mod PatrollerがPlayerを捕まえてもHシーンアニメーションが開始しないことがあります。
...
Patrollerでは、DuplicateしたPrefabを編集する時にInspectorで設定されているStateデータを削除しないでください。特にHシーン対応Prefabでは、BaseControllerの"baseInGameHState"とHSceneControl設定を維持する必要があります。
