...
これらは読むためだけのファイルです。Mod DLLへコンパイルしないでください。
...
4. Sense component: `*.senses.json`。近くのプレイヤーを検知する実用サンプルです。
...
Sense Modでは`Patroller.PatrollerMemory`を使うことが多いです。

Senseを書く時は以下を読んでください。

~~~text
reference/PatrollerMemory_Public_Surface.cs.txt
~~~

`ICaptiveDetector`、`IItemDetector`、`IChainpointDetector`、`ISuspicionCheckpointDetector`、`IPatrollerDetector`でよく使うSource単位の検知メソッドを一覧できます。
