...
Reference-only files are under:

~~~text
reference
~~~

These files are for reading only. Do not compile them into your mod DLL.
...
4. Sense component: `*.senses.json` using a real nearby-player detector example.
...
PatrollerMemory Reference
--------

Sense mods often use `Patroller.PatrollerMemory`.

Read this file when writing a Sense:

~~~text
reference/PatrollerMemory_Public_Surface.cs.txt
~~~

It lists the source-scoped observation methods commonly used by `ICaptiveDetector`, `IItemDetector`, `IChainpointDetector`, `ISuspicionCheckpointDetector`, and `IPatrollerDetector`.
