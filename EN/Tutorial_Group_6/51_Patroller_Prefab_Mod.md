...
H-scene capable patrollers need one more important setup.

If your patroller can play an in-game H scene, the prefab must provide a valid H-scene state path. For normal BaseController-based enemies, this usually means:

1. "BaseController.baseInGameHState" is assigned in the Unity Inspector.
2. The assigned "InGameHState" has its "hsceneControls" list filled.
3. The list contains an HSceneControl entry for every "hIndex" used by that patroller's task options.

Example:

~~~text
Task option config hIndex = 0
  -> baseInGameHState.hsceneControls[0]

Task option config hIndex = 1
  -> baseInGameHState.hsceneControls[1]
~~~

If the prefab uses a custom built-in controller/state pattern, such as Slime, Golem, or Tentacle, preserve that prefab's controller-specific HSceneControl setup instead of replacing it with a blank state.

If this setup is missing, the patroller may catch the player correctly but fail to start the prepared H animation.
...
8. Patroller catches the player but no H scene starts: check "BaseController.baseInGameHState", its "hsceneControls" list, and the task option "hIndex". A console warning like "[InGameHState] Missing HSceneControl index ..." means the list is missing or the index is out of range.
